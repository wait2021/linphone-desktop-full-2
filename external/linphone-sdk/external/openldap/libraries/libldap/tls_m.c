...
#ifdef LDAP_R_COMPILE
static void tlsm_thr_init( void )
{
}
 ...
 */
static int
tlsm_ctx_init( struct ldapoptions *lo, struct ldaptls *lt, int is_server, char *errmsg )
{
	...
	// TODO: parse options in lo and lt
...
// MBedtls v3 deprecated SSLv3, TLS1.0, TLS1.1
	...
	if ( lo->ldo_tls_ciphersuite ) {
		Debug1 (LDAP_DEBUG_ANY, "tlsm_ctx_init Cipher suite selection is not supported by MbedTLS backend, ignore setting %s\n", lt->lt_ciphersuite);
	}
	...
	if ( is_server && lo->ldo_tls_dhfile ) {
		Debug1 (LDAP_DEBUG_ANY, "tlsm_ctx_init DH params from file is not supported by MbedTLS backend, ignore setting %s\n", lo->ldo_tls_dhfile);
	}
...
static int
tlsm_session_accept( tls_session *sess )
{
...
static int
tlsm_session_connect( LDAP *ld, tls_session *sess, const char *name_in )
{
	...
	return tlsm_session_accept(sess);
}

static int
tlsm_session_upflags( Sockbuf *sb, tls_session *sess, int rc )
{
...
static int
tlsm_session_my_dn( tls_session *sess, struct berval *der_dn )
{
...
static int
tlsm_session_peer_dn( tls_session *sess, struct berval *der_dn )
{
	tlsm_session *s = (tlsm_session *)sess;
...
static int
tlsm_session_chkhost( LDAP *ld, tls_session *sess, const char *name_in )
{
	tlsm_session *s = (tlsm_session *)sess;
...
static int
tlsm_session_strength( tls_session *sess )
{
...
{
	Debug0(LDAP_DEBUG_ANY, "tlsm_session_unique channel binding using unique is not available with MbedTLS backend\n");

	return 0;
}

static int
tlsm_session_endpoint( tls_session *sess, struct berval *buf, int is_server )
{
	tlsm_session *s = (tlsm_session *)sess;
...
static int
tlsm_session_pinning( LDAP *ld, tls_session *sess, char *hashalg, struct berval *hash )
{
	...
	if ( hashalg ) {
		...
		if ( mbedtls_hash == NULL ) {
			Debug1( LDAP_DEBUG_ANY, "tlsm_session_pinning: "
	...
	mbedtls_pk_context *pk = (mbedtls_pk_context *)&peer_cert->pk; // remove constness on pk for mbedtls_pk_write_pubkey_der
	...
	unsigned char *digest[64]; // Max size for a hash is set to 64 bytes: SHA512.
...
static int
tlsm_read ( void *ptr, unsigned char *buf, size_t len )
{
...
static int
tlsm_write( void *ptr, const unsigned char *buf, size_t len )
{
...
tlsm_sb_setup( Sockbuf_IO_Desc *sbiod, void *arg )
{
	...
	Sockbuf_IO_Desc *debugsbiod = sbiod;
	while (debugsbiod->sbiod_next) {
		debugsbiod = debugsbiod->sbiod_next;
		if (debugsbiod->sbiod_pvt != NULL) {
	...
	struct tls_data		*p;
	tlsm_session	*session = (tlsm_session *) arg;
	...
	mbedtls_ssl_set_bio(&(session->ssl_ctx), p, tlsm_write, tlsm_read, NULL);
...
tls_impl ldap_int_tls_impl = {
	...
	tlsm_session_unique,
	tlsm_session_endpoint,
	tlsm_session_version,
	tlsm_session_cipher,
	tlsm_session_peercert,
	tlsm_session_pinning,
