   ...
   case ${tgt_os} in
     android*)
       ...
       if [ -z "${COMPILER_LOCATION}" ]; then
         echo "[VPX][BC] gcc not found as ${ANDROID_TOOLCHAIN_PREFIX}-gcc"
         COMPILER_LOCATION=`find "${SDK_PATH}" \
                            -name "${ANDROID_TOOLCHAIN_ALT_PREFIX}-gcc*" -print -quit`
       else
         echo "[VPX][BC] gcc found as ${ANDROID_TOOLCHAIN_PREFIX}-gcc"
         FOUND=1
       fi

       if [ -z "${COMPILER_LOCATION}" ]; then
         echo "[VPX][BC] gcc not found as ${ANDROID_TOOLCHAIN_ALT_PREFIX}-gcc"
         ...
         echo "[VPX][BC] gcc found as ${ANDROID_TOOLCHAIN_ALT_PREFIX}-gcc"
         ANDROID_TEMP_TOOLCHAIN_PREFIX=${ANDROID_TOOLCHAIN_ALT_PREFIX}
         FOUND=1
       fi

       if [ -z "${COMPILER_LOCATION}" ]; then
         echo "[VPX][BC] clang not found as ${ANDROID_TOOLCHAIN_PREFIX}${_ANDROID_NDK_API_INT}-clang"
         COMPILER_LOCATION=`find "${SDK_PATH}/toolchains/llvm/prebuilt/" \
       ...
       if [ -z "${COMPILER_LOCATION}" ]; then
         echo "[VPX][BC] clang not found as ${ANDROID_TOOLCHAIN_ALT_PREFIX}${_ANDROID_NDK_API_INT}-clang"
       fi

       if [ "${USE_GCC}" = "NO" ]; then
         echo "[VPX][BC] Using clang at ${COMPILER_LOCATION%/*}/${ANDROID_TEMP_TOOLCHAIN_PREFIX}-clang"
         ...
         if [ -x ${TOOLCHAIN_PATH}ar ]; then 
           echo "[VPX][BC] ar, as, strip and nm seems to have the same toolchain path than clang"
         else
           echo "[VPX][BC] Couldn't find ar at ${TOOLCHAIN_PATH}ar, using ${COMPILER_LOCATION%/*}/${ANDROID_TOOLCHAIN_PREFIX}-ar instead and same for ld, as, strip and nm"
           TOOLCHAIN_PATH=${COMPILER_LOCATION%/*}/${ANDROID_TOOLCHAIN_PREFIX}-
           ANDROID_TEMP_TOOLCHAIN_PREFIX=${ANDROID_TOOLCHAIN_PREFIX}

           if [ -x ${TOOLCHAIN_PATH}ar ]; then 
             echo "[VPX][BC] ar, as, strip and nm seems to use ANDROID_TOOLCHAIN_PREFIX"
           else
             echo "[VPX][BC] Couldn't find ar at ${TOOLCHAIN_PATH}ar, using ${COMPILER_LOCATION%/*}/llvm-ar instead and same for ld, strip and nm"
             TOOLCHAIN_PATH=${COMPILER_LOCATION%/*}/llvm-
             ANDROID_TEMP_TOOLCHAIN_PREFIX=llvm
             USE_LLVM="YES"
           fi
         fi
         ...
         # Changes required for building with NDK r23b but not r25...
         if [ "${USE_LLVM}" = "YES" ]; then
           if [ -x ${COMPILER_LOCATION%/*}/${ANDROID_TOOLCHAIN_PREFIX}-as ]; then 
             AS=${COMPILER_LOCATION%/*}/${ANDROID_TOOLCHAIN_PREFIX}-as
           else
             if [ -x ${COMPILER_LOCATION%/*}/i686-linux-android-as ]; then 
               AS=${COMPILER_LOCATION%/*}/i686-linux-android-as
               echo "[VPX][BC] ${COMPILER_LOCATION%/*}/${ANDROID_TOOLCHAIN_PREFIX}-as, in case of x86 build with NDK 23 needs to be i686-linux-android instead of x86-linux-android-as"
             fi
           fi
           echo "[VPX][BC] Using as: ${AS}"

           LD=${COMPILER_LOCATION%/*}/ld.lld
           echo "[VPX][BC] Using ld.lld instead of ld: ${LD}"
         else
           if [ -x ${TOOLCHAIN_PATH}ld ]; then 
            echo "[VPX][BC] ld seems to have the same toolchain path than clang"
           else
            LD=${COMPILER_LOCATION%/*}/ld
            echo "[VPX][BC] ld seems to have a different path than other tools: ${LD}"
           fi
         fi
       else
         echo "[VPX][BC] Using gcc at ${COMPILER_LOCATION%/*}/${ANDROID_TEMP_TOOLCHAIN_PREFIX}-gcc"
         TOOLCHAIN_PATH=${COMPILER_LOCATION%/*}/${ANDROID_TEMP_TOOLCHAIN_PREFIX}-
      ...
      case ${tgt_isa} in
        ...
        armv7|armv7s)
          ...
          if [ -f ${SDK_PATH}/source.properties ]; then
             ndk_version=`cat ${SDK_PATH}/source.properties | tr -dc '0-9' | cut -c1-2`
             echo "NDK version is ${ndk_version}"
             if [ -n ${ndk_version} ] && [ ${ndk_version} -ge 25 ]; then
                echo "[VPX][BC] NDK r25 detected, disable neon_asm"
                soft disable neon_asm
             else 
...
EOF
      case ${tgt_cc} in
        gcc*)
          # NDK r23b needs the following to not be called for x86 and X_-_64 targets
          if [ "${USE_LLVM}" = "YES" ]; then
            echo "[VPX][BC] NDK r23b detected, don't set -m${bits} C & LD flag"
          else
            echo "[VPX][BC] NDK < 23 detected"
            add_cflags -m${bits}
            add_ldflags -m${bits}
          fi
          ;;
